...
class DefaultRestartProcessorTest {

  private DefaultRestartProcessor processor;

  @BeforeEach
  void setUp() {
    processor = new DefaultRestartProcessor();
  }

  @Test
  void testCanProcessRestart() {
  ...
  void testProcessRestart() {
    ...
    StepVerifier.create(processor.process(command))
        .assertNext(
            directive -> assertThat(directive).isInstanceOf(WorkflowDirective.Restart.class))
        .verifyComplete();
  }
